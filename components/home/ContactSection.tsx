...
const SOCIALS = [
  ...
  },
  {
    key: 'whatsapp',
    label: 'WhatsApp',
    iconPath: '/reseaux/whatsaap.png',
    iconClassName: 'h-12 w-12',
    url: socialConfig.whatsapp.url,
    handle: socialConfig.whatsapp.handle,
  },
]
            ...
            {SOCIALS.map((social, i) => {
              return (
                <motion.a
                  ...
                  className="group flex min-h-[200px] w-full max-w-[280px] flex-col items-center justify-center gap-4 rounded-[28px] border border-neutral-200/70 bg-white/75 px-6 py-7 text-center shadow-[0_22px_60px_rgba(19,18,16,0.06)] backdrop-blur-sm transition-all duration-300 hover:border-neutral-300 hover:shadow-[0_28px_80px_rgba(19,18,16,0.12)]"
                >
                  <div className="flex h-20 w-20 items-center justify-center overflow-hidden rounded-3xl bg-gradient-to-br from-white to-neutral-50 shadow-[inset_0_1px_0_rgba(255,255,255,0.9),0_16px_34px_rgba(17,17,17,0.08)] transition-transform duration-300 group-hover:scale-105">
                    {'iconPath' in social ? (
                      <img
                        src={social.iconPath}
                        alt={social.label}
                        className={`${'iconClassName' in social && social.iconClassName ? social.iconClassName : 'h-6 w-6'} object-contain`}
                        loading="lazy"
                      />
                    ) : null}
                  </div>
